        ...
        for (int currIndex = prices.size() - 1; currIndex >= 0; currIndex--) {
            for (int currBuy = 0; currBuy <= 1; currBuy++) {
                for (int limit = 1; limit <= 2; limit++) {
                    int case1 = 0;
                    if (currBuy == 1)
                        case1 = -1 * prices[currIndex] + dp[currIndex + 1][0][limit];
                    if (currBuy == 0)
                        case1 = prices[currIndex] + dp[currIndex + 1][1][limit - 1];
                    int case2 = 0 + dp[currIndex + 1][currBuy][limit];
                    dp[currIndex][currBuy][limit] = max(case1,case2);
                }
            }
        }
